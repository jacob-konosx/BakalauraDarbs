...
from api.pieprasijumi import izdzest_uzdevumu_pec_id, dabut_uzdevuma_info_pec_id, sakt_uzdevumu_pec_id, izveidot_karti, augsupieladet_odm_attelus_pec_id
from db.db import db_izveidot_odm_uzdevumu, db_dzest_odm_uzdevumu_pec_id
    ...
    with apstrades_bloks.container():
        ...
        if uzdevuma_dati:
            while not uzdevuma_dati["status"] == 40:
                ...
                with col1_placeholder:
                    st.page_link("pages/ortofoto_parvalde.py", label="Apskatīt izveidoto karti", icon="🗺️")
        else:
            atiestatit_datus()
            st.rerun()
else:
